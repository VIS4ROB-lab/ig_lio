...
class IG_LIO_NODE : public rclcpp::Node {
 public:
  IG_LIO_NODE(std::string package_path)
      : Node("ig_lio_node"),
    ...
    } else if (lidar_type_string == "avia") {
 ...
 private:
  ...
  void DeclareParams() {
    ...
    // Declare and get parameters for GICP constraints, outlier rejection, and
    // others
    this->declare_parameter<double>("odom/gicp_constraints_gain", 100.0);
    this->declare_parameter<double>("odom/point2plane_constraints_gain",
                                    1000.0);
    ...
    this->declare_parameter<std::vector<double>>("extrinsics/imu2lidar/t",
                                                 default_t_imu_lidar);
    this->declare_parameter<std::vector<double>>("extrinsics/imu2lidar/r",
                                                 default_R_imu_lidar);

    this->declare_parameter<std::vector<double>>("extrinsics/robot2imu/t",
                                                 default_t_imu_lidar);
    this->declare_parameter<std::vector<double>>("extrinsics/robot2imu/r",
                                                 default_R_imu_lidar);

    this->declare_parameter<std::vector<double>>("extrinsics/robot2lidar/t",
                                                 default_t_imu_lidar);
    this->declare_parameter<std::vector<double>>("extrinsics/robot2lidar/r",
                                                 default_R_imu_lidar);
  }
  ...
  void GetParams() {
    ...
    this->get_parameter("odom/point2plane_constraints_gain",
                        point2plane_constraints_gain);
    this->get_parameter("odom/enable_undistort", enable_undistort);
    this->get_parameter("odom/enable_outlier_rejection",
                        enable_outlier_rejection);
    this->get_parameter("odom/enable_acc_correct", enable_acc_correct);
    this->get_parameter("odom/enable_ahrs_initalization",
                        enable_ahrs_initalization);
  ...
  void Initialize() {
    ...
    LOG(INFO) << "lidar type: " << lidar_type_string << std::endl
              ...
              << "point2plane_constraints_gain: "
              << point2plane_constraints_gain << std::endl
    ...
    this->extrinsics.robot2imu.t =
        Eigen::Vector3f(robot2imu_t[0], robot2imu_t[1], robot2imu_t[2]);
    this->extrinsics.robot2imu.R =
        Eigen::Map<const Eigen::Matrix<float, -1, -1, Eigen::RowMajor>>(
            std::vector<float>(robot2imu_r.begin(), robot2imu_r.end()).data(),
            3, 3);
    this->extrinsics.robot2imu_T = Eigen::Matrix4f::Identity();
    this->extrinsics.robot2imu_T.block(0, 3, 3, 1) =
        this->extrinsics.robot2imu.t;
    this->extrinsics.robot2imu_T.block(0, 0, 3, 3) =
        this->extrinsics.robot2imu.R;

    this->extrinsics.robot2lidar.t =
        Eigen::Vector3f(robot2lidar_t[0], robot2lidar_t[1], robot2lidar_t[2]);
    this->extrinsics.robot2lidar
        .R = Eigen::Map<const Eigen::Matrix<float, -1, -1, Eigen::RowMajor>>(
        std::vector<float>(robot2lidar_r.begin(), robot2lidar_r.end()).data(),
        3, 3);

    this->extrinsics.robot2lidar_T = Eigen::Matrix4f::Identity();
    this->extrinsics.robot2lidar_T.block(0, 3, 3, 1) =
        this->extrinsics.robot2lidar.t;
    this->extrinsics.robot2lidar_T.block(0, 0, 3, 3) =
        this->extrinsics.robot2lidar.R;
  ...
  void Topics() {
    ...
    imu_sub_ = this->create_subscription<sensor_msgs::msg::Imu>(
        imu_topic, rclcpp::ServicesQoS(),
        std::bind(&IG_LIO_NODE::ImuCallBack, this, _1));
    if (lidar_type_ == LidarType::LIVOX) {
      cloud_sub_ = nullptr;
      livox_sub_ = this->create_subscription<livox_ros_driver2::msg::CustomMsg>(
          lidar_topic, rclcpp::ServicesQoS(),
          std::bind(&IG_LIO_NODE::LivoxCloudCallBack, this,
                    std::placeholders::_1));
    } else {
      livox_sub_ = nullptr;
      cloud_sub_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
          lidar_topic, rclcpp::ServicesQoS(),
          std::bind(&IG_LIO_NODE::CloudCallBack, this, std::placeholders::_1));
    }

    // Setup publishers
    odom_pub_ =
        this->create_publisher<nav_msgs::msg::Odometry>("/lio/odometry", 10);
    current_scan_pub_ = this->create_publisher<sensor_msgs::msg::PointCloud2>(
        "/current_scan", 10);
    keyframe_scan_pub_ = this->create_publisher<sensor_msgs::msg::PointCloud2>(
        "/keyframe_scan", 10);
    cloud_registered_pub_ =
        this->create_publisher<sensor_msgs::msg::PointCloud2>(
            "/cloud_registered", 10);
    pubOdomToMap_ =
        this->create_publisher<nav_msgs::msg::Odometry>("/lio/odom_to_map", 20);
  ...
  void ImuCallBack(const sensor_msgs::msg::Imu::SharedPtr msg_ptr) {
    ...
    if (std::abs(timediff_lidar_wrt_imu) > 0.1 && timediff_correct_flag) {
      // In ROS2, directly manipulate the builtin_interfaces::msg::Time
      // structure
      auto corrected_time =
          rclcpp::Time(imu_msg.header.stamp) +
          rclcpp::Duration::from_seconds(timediff_lidar_wrt_imu);
      ...
      imu_msg.header.stamp.nanosec =
          corrected_time.nanoseconds() % static_cast<uint64_t>(1e9);
    }
    {
      ...
      imu_msg.linear_acceleration.x = msg_ptr->linear_acceleration.x * a +
                                      last_imu.linear_acceleration.x * b;
      imu_msg.linear_acceleration.y = msg_ptr->linear_acceleration.y * a +
                                      last_imu.linear_acceleration.y * b;
      imu_msg.linear_acceleration.z = msg_ptr->linear_acceleration.z * a +
                                      last_imu.linear_acceleration.z * b;
      last_imu = *msg_ptr;

      // Some Livox datasets miss the gravitational constant in the
      // accelerometer
    ...
    timer.Evaluate(
        [&]() {
...

          {
            ...
            cloud_buff.push_back(std::make_pair(
                msg->header.stamp.sec + msg->header.stamp.nanosec * 1e-9,
                cloud_ptr));
          }
        ...
        [&]() {
...

          {
            ...
            cloud_buff.push_back(
                std::make_pair(first_scan_timestamp, cloud_ptr));
  ...
  bool SyncMeasurements() {
    ...
    if (!measurement_pushed) {
      if (!process_lidar) {
        ...
        *cloud_sort = *cloud_buff.front().second;
        ...
        } else if (local_sensor_measurement.cloud_ptr_->points.back()
                           .curvature /
                       (double)(1000) <
      ...
      } else if (sensor_measurement.measurement_type_ ==
    ...
    if (imu_buff.back().header.stamp.sec +
      ...
      if (debug_) {
        ...
        double imu_time = imu_buff.back().header.stamp.sec +
                          imu_buff.back().header.stamp.nanosec * 1e-9;
        double time_difference = sensor_measurement.lidar_end_time_ -
                                 imu_time;  // This is how much the LIDAR end
                                            // time is ahead of the IMU time

        // Log the time difference
        LOG(WARNING) << "this is the timer issue for lidar time and imu timer. "
                        "Difference: "
  ...
  void Process() {
    ...
    if (!SyncMeasurements()) {
      // if (debug_) LOG(WARNING) << "failed to do SyncMeasurements" <<
      // std::endl;
      return;
    }

    // Step 2: Use AHRS or static initialization
    // If the imu message has orientation channel, LIO can be initialized via
    // AHRS
    if (!lio_ptr->IsInit()) {
    ...
    for (size_t i = 0; i < sensor_measurement.imu_buff_.size(); ++i) {
      ...
      if (i == sensor_measurement.imu_buff_.size() - 1) {
        ...
        time = sensor_measurement.imu_buff_.at(i).header.stamp.sec +
               sensor_measurement.imu_buff_.at(i).header.stamp.nanosec * 1e-9;
      }
      Eigen::Vector3d acc(
    ...
    // // // odometry message
    auto nanosec_part = static_cast<uint32_t>(
        (sensor_measurement.lidar_end_time_ -
         static_cast<uint64_t>(sensor_measurement.lidar_end_time_)) *
        1e9);
    ...
    if (sendTF) publishToRos(result_pose, temp_q, current_time_stamp);
    ...
    // double norm_ =
    //     Sophus::SO3d(lio_ptr->correctRotationMatrix(delta_p.block<3, 3>(0,
    //     0)))
    //         .log()
    //         .norm();
    ...
    //     norm_ > 0.18) {
    //   if (debug_) LOG(INFO) << "Done checking delta p" << std::endl;
  ...
  void publish_odom_To_map(const nav_msgs::msg::Odometry& odom) {
    ...
    try {
      transform_stamped = tf_buffer_.lookupTransform(
          map_frame, odom_pose.header.frame_id, odom_pose.header.stamp,
          rclcpp::Duration::from_seconds(1.0));
      tf2::doTransform(odom_pose, map_pose, transform_stamped);
    } catch (tf2::TransformException& ex) {
      // RCLCPP_WARN(this->get_logger(), "Could not transform cloud: %s",
      // ex.what());
      return;
    }
  ...
  void publishToRos(Eigen::Matrix4d& result_pose, Eigen::Quaterniond& temp_q,
                    rclcpp::Time& current_time_stamp) {
    ...
    transformStamped.transform.translation.x =
        this->extrinsics.robot2lidar.t[0];
    transformStamped.transform.translation.y =
        this->extrinsics.robot2lidar.t[1];
    transformStamped.transform.translation.z =
        this->extrinsics.robot2lidar.t[2];
  ...
  std::vector<double> default_R_imu_lidar = {
